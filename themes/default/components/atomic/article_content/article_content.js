...
document.addEventListener('DOMContentLoaded', function() {
  const articleContentComponents = document.querySelectorAll('[data-component="article_content"]');
...
function initializeReadingProgress(component) {
  ...
  function updateProgress() {
    ...
    if (scrollTop > articleTop) {
      ...
      progress = Math.min(100, Math.max(0, (articleScrollTop / articleHeight) * 100));
    }
...
function initializeTableOfContents(component) {
  ...
  const headings = component.querySelectorAll('.article-body h2, .article-body h3, .article-body h4');
